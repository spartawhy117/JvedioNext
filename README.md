...
- 收藏页继续保留单个 `收藏` 导航入口，不拆成多个一级导航
- 默认进入收藏首页，并通过 `影片 / 标签 / 演员` 三张入口卡分流
- 收藏影片页复用影片卡与详情返回链，支持搜索、排序与批量操作
- 收藏标签页与收藏演员页复用各自已有页面能力，统一承接收藏状态、批量收藏和批量取消收藏

---

## 标签页浏览

- 标签页仅对 `MetaTube 标准库` 开放
- 当前支持 `类型 / 系列 / 厂商` 三种标签维度
- 可从左侧导航统一浏览，也可从影片详情页点击对应标签跳转
- 支持在“全部标准库”与单库范围内查看同类影片
...
- **标准库**
  - 仅明确命中分集规则时，才会在同一数据源内整理到同一个 `VID` 目录
...
- 可配置最多 3 条扫描目录，扫描负责识别影片并入库，抓取负责补齐海报、NFO、演员与详情页
- 扫描只会把明确命中分集规则的文件整理到同一个 `VID` 目录，例如 `_1`、`(2)`、`CD1`、`PART2`；普通单文件和未命中规则的同 `VID` 文件保持原位
- 新版本扫描阶段**不会自动删除用户磁盘上的原影片文件**；同 `VID` 多文件会各自保留、各自入库，并在列表中按**一文件一张卡**展示
- 抓取后的 `NFO` 与三张主图会写回影片所在目录
