...
- **非标准本地库**：面向论坛或社区下载的国产、OF 等非标准 JAV 电影数据，保留原目录结构，按文件夹层级自动组织合集，并支持自定义标签管理
...
- 你有从论坛或社区下载的国产、OF 等非标准 JAV 数据，希望不改原目录，按文件夹层级自动组织合集并使用标签管理
...
| 非标准本地库 | 论坛或社区下载的国产、OF 等非标准 JAV 数据 | 不需要 | 按文件夹层级自动组织合集，不搬运 | 不删除原目录和原文件 |
...
- 标准库集中展示抓取状态、资源缺口、规则问题、失效记录和分组一致性，并保留最近一次健康结果
- 健康检查会在后台运行，慢盘或外置存储唤醒时不会卡住健康页；失败时仍保留上一次可用结果
...
| 基本 | 主题、语言、关闭行为、调试模式、健康检查存储唤醒等待时间 |
| 显示 | 影片卡大小、海报样式、影片库和演员页默认排序 |
| 播放器设置 | 自定义播放器路径，未配置时回退系统默认 |
| 标准库 | MetaTube 服务、请求超时、JavDB/JavBus 搜索站点地址、恢复默认和连通性测试 |
...
适合从论坛或社区下载的国产、OF 等非标准 JAV 电影数据。这类资源通常没有稳定番号、没有可直接搜刮的标准元数据；软件保留原目录结构，并按文件夹层级自动组织合集。升级后的本地库不再使用旧合集规则，文件夹组织示例与逐层浏览规则见 [《软件使用指南》](https://github.com/spartawhy117/JvedioNext/wiki/User-Guide#33-按目录浏览合集)。

- 配置 1 条扫描目录；`刷新目录` 只同步文件变化，不移动、不重命名、不按 `VID` 整理原文件
- 当前目录的直接影片平铺显示；自身或后代含已导入影片的直接子目录显示为合集卡，可逐层进入
- 空目录不显示；不再配置“合集目录列表”或“下一层子目录按合集显示”等旧规则
- `FFmpeg` 会生成静态封面和悬停预览；缓存只在软件数据目录，不改动原影片目录
- `编辑` 可修改库名和扫描目录；`移除库` 只清理软件内数据和缓存，不删除原影片
...
> 自动更新：2026/09/21 14:39（Asia/Shanghai）

累计：版本发布数 67，已完成 Issue 82，未计划 Issue 18

当周（最近 7 天）：版本发布数 1，已完成 Issue 3，未计划 Issue 2
<!-- repo-report:end -->
