...
- **演员页**：支持补全演员扩展信息，并可按年龄、身高和罩杯筛选演员结果
- **收藏页**：支持影片、标签和演员统一收藏，方便集中回看常用内容
- **标签页**：支持按 `类型 / 系列 / 厂商 / 自定义` 浏览标签，并继续展开关联影片
- **搜索页**：支持按单个 `VID`、演员原词和单个 `Tag` 原词搜索远端影片，并直接查看本地入库状态
...
- 标准库当前支持：常见标准番号、`FC2-PPV`、常见数字前缀素人番号、部分带厂商标识的无码番号
- 数字前缀素人番号与部分无码样本当前已补充更多识别范围；具体命名与搜索口径请查看完整命名规则
- 素人番号与无码样本的抓取行为会按当前规则做兼容搜索或失败保留，详细边界见命名规则文档
...
<a id="actors-overview"></a>
## 演员页概览

- 演员页现在支持补全扩展信息，可获取年龄、身高和罩杯三项数据。
- 支持按年龄、身高和罩杯筛选演员结果，也可继续按作品数量和头像状态收口列表。
- 页面顶部会直接显示已补全、未补全和当前补全过程进度，批量补全过程中也可以主动停止任务。
- 收藏演员页会继续复用同一套扩展信息摘要与筛选能力，方便按收藏对象继续浏览。
...
- 可以按单个 `VID`、单个演员原词和单个 `Tag` 原词搜索远端影片，并直接查看 `标准库已存在 / 本地库已存在 / 已收藏 / 未入库` 四类本地状态
- 结果页会优先展示本地状态映射，方便快速判断是否需要继续入库
...
> 自动更新：2026/07/06 13:48（Asia/Shanghai）

累计：版本发布数 55，已完成 Issue 33，未计划 Issue 7

当周（最近 7 天）：版本发布数 1，已完成 Issue 0，未计划 Issue 0
<!-- repo-report:end -->
